...
def request_demo_view(request):
    ...
    subject = "New Enquirey for Earthing Studies, Lightning Protection & EMFI Analysis"
    text_body = (
    ...
    html_body = f"""
        <h2 style="margin:0 0 8px">New Enquirey for Earthing Studies, Lightning Protection & EMFI Analysis</h2>
...
def contact_section(request):
    ...
    if request.method == "POST" and form.is_valid():
        ...
        # Email body
        subject = "New contact submission for Earthing Studies, Lightning Protection & EMFI Analysis"
        text_body = "\n".join(
            [
                "New contact submission for Earthing Studies, Lightning Protection & EMFI Analysis:",
...
def contact_block_submit(request):
    ...
    # --- Email notification ---
    subject = f"[Earthing Studies, Lightning Protection & EMFI Analysis] Consulting request: {name}"
 
    text_body = "\n".join([
        "A new consulting request was submitted for Earthing Studies, Lightning Protection & EMFI Analysis:",
        f"Name: {name}",
        f"Email: {email}",
